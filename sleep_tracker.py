...
def extract_sleep_data(zip_bytes, existing_dates):
    ...
    try:
        ...
        for sess in c.fetchall():
            ...
            if bedtime in existing_dates:
                print(f"Skipping {bedtime}, already in sheet")
                continue
...
def main():
    try:
        ...
        row_data = extract_sleep_data(zip_bytes, existing_dates)
        if not row_data:
            print("No new sleep data found")
            return

        append_to_sheet(worksheet, row_data)
        print("Done!")
    except Exception as e:
        print(f"Error: {e}")
        import traceback
